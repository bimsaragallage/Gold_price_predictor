...
model1.add(SimpleRNN(units=10,activation='relu'))
model1.add(Dropout(0.2))
model1.add(Dense(1,activation='linear'))


cp1 = ModelCheckpoint('model1/', monitor='val_accuracy', save_best_only=True)
model1.compile(loss=MeanSquaredError(), optimizer=Adam(learning_rate=0.001), metrics=[RootMeanSquaredError()])

training_model = model1.fit(X_train1, y_train1, validation_data=(X_val1, y_val1), epochs=50,batch_size = 10, callbacks=[cp1])

model1 = load_model('model1/')

try:
    previous_best_model = keras.models.load_model('previous_best_model.h5')
    if model1.evaluate(X_val1, y_val1)[1] > previous_best_model.evaluate(X_val1, y_val1)[1]:
        print("New model has better accuracy. Saving the new model.")
        model1.save('previous_best_model.h5')
    else:
        print("Previous best model remains the best.")
except:
    # If there is no previous best model, save the current best model
    print("No previous best model found. Saving the current best model.")
    model1.save('previous_best_model.h5')
